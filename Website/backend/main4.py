...
# storage.child(path_on_cloud).put(path_locao)

test_id = 'cEmDKustQ5g7SFrwruRmxlQI7622'

USER = {"idToken": None, "localId": None}
...
def personal_portal():
    db = firebase.database()
    if USER["idToken"] != None and USER["localId"] != None:
        id = USER["localId"]
...
def view_file():
    ...
    return render_template('view-pdfs.html', urls=display_urls)
